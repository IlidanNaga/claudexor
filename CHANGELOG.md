...
- **Unreleased** — The Codex HTTP model transport now declares its own verified client version when it reads an account's model catalog (`CODEX_HTTP_CLIENT_VERSION`, raised to a newer installed Codex CLI, never the managed-installer pin). The backend lists a model only for clients at or above that model's minimum version, so a release that only moved the installer used to hide newly floored models such as `gpt-6-sol` from this route although generation already served them (#339). The negotiated account view carries `clientVersion` and `clientVersionSource` per catalog (the legacy raw-catalog query keeps its shape), both membership refusals name the declared version, and the constant moves with a recorded catalog fixture pair whose shared rows must stay identical.
- **Unreleased** — Whether a model list may refuse an explicit model is now the harness's own declaration, honoured on every list it owns. `model_inventory_absence` used to govern only a live `models()` answer, while every manifest hint list stayed strict by construction: a settings write, the doctor's configured-model check and the explicit reviewer panel refused any id the shipped list lacked (and `claudexor models` could show nothing beyond that list), so a new vendor model needed a Claudexor release to become usable through them (#338, #340). Claude, Codex and Cursor declare `advisory`: their lists prove presence, never absence, so an unlisted explicit model is persisted or forwarded byte-identical and the vendor decides, with the admission said once — the settings read-back carries `notes` (the CLI prints them), the readiness row carries the note in its detail (the text `doctor` prints it too), the per-spawn gate keeps its status event; the agent capability catalog's `configuredModelValid` stays a boolean that reports such an admission as valid without the note. raw-api, agy and opencode stay authoritative and refuse as before; the automatic reviewer panel keeps skipping an unlisted family at zero cost; HTTP model operations stay strict against the account catalog. `validateModel` takes the list, its source and the declaration with no defaults, and one registry gate (`checkHarnessModel`) replaces four hand-built copies. Refusals now state observations (which account supplied which list) rather than guessing a cause. Invariants: INV-104 amended (CONCEPT-CHANGE) — absence is a per-harness declaration; new canary `[INV-104:settings-write-advisory]`, the strict canary moves to agy.
- **Unreleased** — Claude models are discovered from the installed binary instead of a shipped list. The claude adapter's `models()` runs the prompt-free `initialize` handshake of the installed `claude` (never `--model`, no settings, no MCP, model-override env scrubbed) and returns the picker's selectors with their vendor-reported resolutions as pinnable exact ids (`origin: live`, `resolved_model`), plus the frozen hint ids (`origin: hint`). A `config_dir_login` profile is probed under its own config dir and an `api_key`/`oauth_token` profile with its own credential in the env var its runs use, so the account view carries the account's rows; the unscoped listing is a credential-free binary probe. A failing binary yields the hint rows and the listing reports `source: manifest` with the frozen stamp, never a live claim. The probe and the `--help` effort memo are keyed by the binary's identity (realpath, inode, size, mtime — `harnessBinaryIdentity` in core), so an in-place CLI update is re-read without a daemon restart; a run whose env patch carries a PATH has its effort ladder, readonly flag set and `--version` read from the binary that PATH selects. Claude declares `model_inventory_absence: advisory`; `claudexor models` prints resolutions and hint marks. Invariants: none amended beyond the sibling INV-104 change; INV-020/INV-022 hold (`origin`/`resolved_model` are produced by the claude adapter and read by the CLI and the registry).
- **Unreleased** — Managed vendor pins catch up to Claude Code 2.1.281 and Codex 0.156.1, so a fresh managed Codex install can run GPT-6 Sol and GPT-6 Luna, which Codex 0.153.3 neither lists nor runs on a ChatGPT account. Both models join the Codex manifest hints and the recorded effort snapshot from a live 0.156.1 `model/list` capture (the default model stays GPT-6 Astra; ladders of models this account no longer lists stay under the snapshot's union rule). The Claude `--help` effort ladder is unchanged on 2.1.281. Credential-free stream recordings were re-captured from the pinned binaries (Claude controlled-endpoint probes; Codex loopback probes, byte-identical to 0.153.3), while recordings that need a paid live run or a real vendor incident keep the version they were captured from. The Claude known-model hint list keeps its last actual verification stamp (2.1.261) rather than following the pin. An already-installed CLI is not replaced (on an install that keeps 0.153.3 / 2.1.261 the recorded effort snapshot no longer vouches for that CLI, so a run whose live effort probe fails proceeds at the vendor default with the ignored-settings disclosure); the app compatibility floor is unchanged.
