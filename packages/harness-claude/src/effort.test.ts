...
describe("the claude ladder is a property of the INSTALLED binary", () => {
  it("keeps the release snapshot equal to the recorded 2.1.281 help ladder", () => {
    const help = readFileSync(new URL("../fixtures/help-2.1.281.txt", import.meta.url), "utf8");
    expect(CLAUDE_EFFORT_SNAPSHOT_VERIFIED_AGAINST).toBe("2.1.281");
...
describe("the claude effort probe degrades gracefully", () => {
  ...
  it("snapshot trust is exact-version, and an unknown/unparseable version never vouches", () => {
    expect(claudeSnapshotTrustedForVersion("2.1.281")).toBe(true);
    expect(claudeSnapshotTrustedForVersion("2.1.281 (Claude Code)")).toBe(true);
    ...
    expect(claudeSnapshotTrustedForVersion("2.1.281.1")).toBe(false);
    expect(claudeSnapshotTrustedForVersion("2.1.2810")).toBe(false);
